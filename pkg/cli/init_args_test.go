	...
	Describe("generateArgsFile", func() {
		Context("with local template repository", func() {
			It("should generate YAML args file", func() {
				...
				Expect(err).NotTo(HaveOccurred())
				Expect(string(envContent)).To(ContainSubstring("BLCLI_TERRAFORM_ORGANIZATION_ID=0"))
				Expect(string(envContent)).To(ContainSubstring("BLCLI_TERRAFORM_BILLING_ACCOUNT_ID=01ABCD-2EFGH3-4IJKL5"))
				Expect(string(envContent)).To(ContainSubstring("BLCLI_GITOPS_PRD_HELLO_WORLD_APPLICATION_REPO=https://github.com/your-org/gitops.git"))
				Expect(string(envContent)).To(ContainSubstring("BLCLI_GITOPS_STG_HELLO_WORLD_APPLICATION_REPO=https://github.com/your-org/gitops.git"))
				Expect(string(envContent)).To(ContainSubstring("BLCLI_GITOPS_STG_HELLO_WORLD_2_APPLICATION_REPO=https://github.com/your-org/gitops.git"))
