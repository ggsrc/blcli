...
You can then customize the generated file and use it with blcli init --args.`,
  ...
  # Use a template profile overlay (e.g. bl-template-one profiles/full)
  blcli init-args ./bl-template-one --profile full -o args.yaml

  # Interactive wizard with preview and confirmation
  blcli init-args ./bl-template --wizard

  # Preview generated configuration without writing files
  blcli init-args ./bl-template --preview -o workspace/config/args.yaml`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if useWizard {
				answers, err := runInitArgsWizard(wizardAnswers{
					Org:        org,
					Profile:    profile,
			...
			return generateArgsFile(templateRepoValue, outputPath, finalFormat, org, profile, forceUpdate, cacheExpiry, previewOnly, wizard)
		},
	}
	...
	cmd.Flags().StringVar(&profile, "profile", "minimal",
		"Template profile overlay name (default: minimal). Loads profiles/<profile>/*.yaml on top of module default.yaml files")
	...
	cmd.Flags().BoolVar(&useWizard, "wizard", false,
		"Interactive wizard to collect org, profile, workspace, and GCP billing settings")
	cmd.Flags().BoolVar(&previewOnly, "preview", false,
		"Preview generated args configuration without writing files")

	return cmd
}

// generateArgsFile generates an args configuration file from default.yaml files in template repository
func generateArgsFile(repoURL, outputPath, format, org, profile string, forceUpdate bool, cacheExpiry time.Duration, previewOnly bool, wizard *wizardAnswers) error {
	...
	if profile != "" && profile != "minimal" {
		if err := mergeProfileOverlays(loader, allDefaultData, profile); err != nil {
			return err
		}
	}
	...
	if wizard != nil {
		applyWizardOverrides(&argsConfig, *wizard)
	}
	...
	printArgsConfigPreview(&argsConfig, outputPath)
	...
	if wizard != nil && isInteractive() {
		ok, err := confirmWritePreview()
		if err != nil {
			return err
		}
		if !ok {
...
func writeInitArgsEnvFile(path string, data *ArgsConfig) error {
	...
	lines = append(lines,
		...
		"# Each variable below overrides the matching field in args.yaml at init time.",
		"",
		"# --- Terraform (GCP) ---",
	)
	lines = appendEnvVar(lines,
		[]string{
			"GCP Organization ID. Use 0 if you do not have an Organization (personal account).",
			"Maps to: terraform.global.OrganizationID",
		},
		"BLCLI_TERRAFORM_ORGANIZATION_ID",
		envValues["BLCLI_TERRAFORM_ORGANIZATION_ID"],
	)
	lines = append(lines, "")
	lines = appendEnvVar(lines,
		[]string{
			"GCP Billing Account ID. Required before apply terraform.",
			"Format: 01XXXX-YYYYYY-ZZZZZZ (find in Cloud Console -> Billing).",
			"Maps to: terraform.global.BillingAccountID",
		},
		"BLCLI_TERRAFORM_BILLING_ACCOUNT_ID",
		envValues["BLCLI_TERRAFORM_BILLING_ACCOUNT_ID"],
	)

	gitopsApps := extractGitopsAppEnvEntries(data)
	if len(gitopsApps) > 0 || strings.TrimSpace(envValues["BLCLI_GITOPS_SOURCE_REPO_URL"]) != "" {
		lines = append(lines, "", "# --- GitOps ---")
		lines = appendEnvVar(lines,
			[]string{
				"GitOps manifest repository URL (where application YAML lives).",
				"Example: https://github.com/your-org/gitops.git or git@github.com:your-org/gitops.git",
				"Maps to: gitops.apps[].project[].parameters.SourceRepoURL",
			},
			"BLCLI_GITOPS_SOURCE_REPO_URL",
			envValues["BLCLI_GITOPS_SOURCE_REPO_URL"],
		)
		lines = append(lines, "")
	}

	for _, appEnv := range gitopsApps {
		lines = append(lines, fmt.Sprintf("# GitOps app %s/%s", appEnv.ProjectName, appEnv.AppName))
		lines = appendEnvVar(lines,
			[]string{
				"Application source repository for this app (image/chart repo referenced by GitOps).",
				"Example: https://github.com/your-org/hello-world.git",
				fmt.Sprintf("Maps to: gitops.apps[name=%s].repo / ApplicationRepo", appEnv.AppName),
			},
			fmt.Sprintf("BLCLI_GITOPS_%s_%s_APPLICATION_REPO",
				strings.ToUpper(appEnv.ProjectKey),
				strings.ToUpper(appEnv.AppKey),
			),
			appEnv.ApplicationRepo,
		)
		lines = append(lines, "")
	}

	if hasArgocdComponent(data) {
		lines = append(lines, "# --- ArgoCD (shared) ---")
		lines = appendEnvVar(lines,
			[]string{
				"Git repository URL ArgoCD uses to sync manifests. Prefer SSH: git@github.com:org/gitops.git",
				"HTTP URLs may cause SSH auth failures unless configured otherwise.",
				"Maps to: kubernetes.*.argocd GitRepositoryURL",
			},
			"BLCLI_ARGOCD_GIT_REPOSITORY_URL",
			envValues["BLCLI_ARGOCD_GIT_REPOSITORY_URL"],
		)
		lines = append(lines, "")

		for _, projectName := range extractArgocdProjectNames(data) {
			projectKey := strings.ToUpper(projectName)
			lines = append(lines, fmt.Sprintf("# --- ArgoCD (%s) ---", projectName))
			lines = appendEnvVar(lines,
				[]string{
					"Public ArgoCD UI URL for this environment.",
					"Example: https://app.example.com/argocd (test: keep placeholder or use http://127.0.0.1 with port-forward).",
					fmt.Sprintf("Maps to: kubernetes.projects[name=%s].argocd argocd-url", projectName),
				},
				fmt.Sprintf("BLCLI_ARGOCD_%s_URL", projectKey),
				envValues[fmt.Sprintf("BLCLI_ARGOCD_%s_URL", projectKey)],
			)
			lines = append(lines, "")
			lines = appendEnvVar(lines,
				[]string{
					"GitHub OAuth App Client ID for ArgoCD Dex login.",
					"Create at: GitHub -> Settings -> Developer settings -> OAuth Apps.",
				},
				fmt.Sprintf("BLCLI_ARGOCD_%s_DEX_GITHUB_CLIENT_ID", projectKey),
				envValues[fmt.Sprintf("BLCLI_ARGOCD_%s_DEX_GITHUB_CLIENT_ID", projectKey)],
			)
			lines = append(lines, "")
			lines = appendEnvVar(lines,
				[]string{
					"GitHub OAuth App Client Secret for ArgoCD Dex login.",
				},
				fmt.Sprintf("BLCLI_ARGOCD_%s_DEX_GITHUB_CLIENT_SECRET", projectKey),
				envValues[fmt.Sprintf("BLCLI_ARGOCD_%s_DEX_GITHUB_CLIENT_SECRET", projectKey)],
			)
			lines = append(lines, "")
			lines = appendEnvVar(lines,
				[]string{
					"Comma-separated GitHub org(s) allowed to log in via Dex.",
					"Example: your-github-org",
				},
				fmt.Sprintf("BLCLI_ARGOCD_%s_DEX_GITHUB_ORGS", projectKey),
				envValues[fmt.Sprintf("BLCLI_ARGOCD_%s_DEX_GITHUB_ORGS", projectKey)],
			)
			lines = append(lines, "")
			lines = appendEnvVar(lines,
				[]string{
					"RBAC group prefix for ArgoCD role bindings (e.g. SiriusPlatform).",
					"Maps to: kubernetes.*.argocd RBACRoleBindings group prefix.",
				},
				fmt.Sprintf("BLCLI_ARGOCD_%s_RBAC_GROUP_PREFIX", projectKey),
				envValues[fmt.Sprintf("BLCLI_ARGOCD_%s_RBAC_GROUP_PREFIX", projectKey)],
			)
			lines = append(lines, "")
		}
	}
...
func hasArgocdComponent(data *ArgsConfig) bool {
	if data == nil {
		return false
	}
	for _, project := range data.Kubernetes.Projects {
...
func extractArgocdProjectNames(data *ArgsConfig) []string {
	if data == nil {
		return nil
	}
	var result []string
...
// appendEnvVar appends documentation comments and one KEY=VALUE line to a .env file body.
func appendEnvVar(lines []string, docs []string, key, value string) []string {
	for _, doc := range docs {
		lines = append(lines, "# "+doc)
	}
	lines = append(lines, fmt.Sprintf("%s=%s", key, envQuote(value)))
	return lines
}
...
func mergeProfileOverlays(loader *template.Loader, allDefaultData map[string]interface{}, profile string) error {
	modules := []string{"terraform", "kubernetes", "gitops"}
	foundAny := false
	for _, moduleName := range modules {
		overlayPath := filepath.ToSlash(filepath.Join("profiles", profile, moduleName+".yaml"))
		if !loader.CacheExists(overlayPath) {
			continue
		}
		foundAny = true
		fmt.Printf("  Loading profile overlay: %s\n", overlayPath)
		content, err := loader.LoadTemplate(overlayPath)
		if err != nil {
			return fmt.Errorf("failed to load profile overlay %s: %w", overlayPath, err)
		}
		var overlay map[string]interface{}
		if err := yaml.Unmarshal([]byte(content), &overlay); err != nil {
			return fmt.Errorf("failed to parse profile overlay %s: %w", overlayPath, err)
		}
		base, _ := allDefaultData[moduleName].(map[string]interface{})
		allDefaultData[moduleName] = mergeModuleDefaultData(base, overlay)
	}
	if !foundAny {
		return fmt.Errorf("profile %q not found: no overlays under profiles/%s/", profile, profile)
	}
	return nil
}

func mergeModuleDefaultData(base, overlay map[string]interface{}) map[string]interface{} {
	if overlay == nil {
		return base
	}
	if base == nil {
	...
	if globalOverlay, ok := overlay["global"].(map[string]interface{}); ok {
		result["global"] = mergeStringMaps(toStringMap(result["global"]), globalOverlay)
	}
	if initOverlay, ok := overlay["init"].(map[string]interface{}); ok {
		result["init"] = mergeInitSection(toStringMap(result["init"]), initOverlay)
	}
	if projectsOverlay, ok := overlay["projects"].([]interface{}); ok {
		baseProjects, _ := result["projects"].([]interface{})
		result["projects"] = mergeProjectsYAML(baseProjects, projectsOverlay)
	}
	for _, key := range []string{"argocd", "apps"} {
...
func mergeInitSection(base map[string]interface{}, overlay map[string]interface{}) map[string]interface{} {
	if base == nil {
		return overlay
	}
	result := deepCopyMap(base)
	if componentsOverlay, ok := overlay["components"].(map[string]interface{}); ok {
		baseComponents := toStringMap(result["components"])
		result["components"] = mergeStringMaps(baseComponents, componentsOverlay)
	}
	return result
}

func mergeProjectsYAML(baseProjects, overlayProjects []interface{}) []interface{} {
	if len(overlayProjects) == 0 {
		return baseProjects
	}
	if len(baseProjects) == 0 {
		return overlayProjects
	}
	result := make([]interface{}, len(baseProjects))
	copy(result, baseProjects)
	...
	for _, item := range overlayProjects {
		overlayProject, ok := item.(map[string]interface{})
		if !ok {
			continue
		}
		name, _ := overlayProject["name"].(string)
		if name == "" {
			continue
		}
		if idx, exists := indexByName[name]; exists {
			baseProject, _ := result[idx].(map[string]interface{})
			result[idx] = mergeProjectYAML(baseProject, overlayProject)
			continue
		}
		result = append(result, overlayProject)
		indexByName[name] = len(result) - 1
	}
	return result
}

func mergeProjectYAML(baseProject, overlayProject map[string]interface{}) map[string]interface{} {
	if baseProject == nil {
		return overlayProject
	}
	result := deepCopyMap(baseProject)
	if globalOverlay, ok := overlayProject["global"].(map[string]interface{}); ok {
		result["global"] = mergeStringMaps(toStringMap(result["global"]), globalOverlay)
	}
	if id, ok := overlayProject["id"].(string); ok && id != "" {
		result["id"] = id
	}
	if componentsOverlay, ok := overlayProject["components"].([]interface{}); ok {
		baseComponents, _ := result["components"].([]interface{})
		result["components"] = mergeComponentsYAML(baseComponents, componentsOverlay)
	}
	return result
}

func mergeComponentsYAML(baseComponents, overlayComponents []interface{}) []interface{} {
	if len(overlayComponents) == 0 {
		return baseComponents
	}
	if len(baseComponents) == 0 {
		return overlayComponents
	}
	result := make([]interface{}, len(baseComponents))
	copy(result, baseComponents)
	indexByName := make(map[string]int)
	for i, item := range result {
		componentMap, ok := item.(map[string]interface{})
		if !ok {
			continue
		}
	...
	for _, item := range overlayComponents {
		overlayComponent, ok := item.(map[string]interface{})
		if !ok {
			continue
		}
		name, _ := overlayComponent["name"].(string)
		if name == "" {
			continue
		}
		if idx, exists := indexByName[name]; exists {
			baseComponent, _ := result[idx].(map[string]interface{})
			result[idx] = mergeComponentYAML(baseComponent, overlayComponent)
			continue
		}
		result = append(result, overlayComponent)
		indexByName[name] = len(result) - 1
	}
	return result
}

func mergeComponentYAML(baseComponent, overlayComponent map[string]interface{}) map[string]interface{} {
	if baseComponent == nil {
		return overlayComponent
	}
	result := deepCopyMap(baseComponent)
	if paramsOverlay, ok := overlayComponent["parameters"].(map[string]interface{}); ok {
		result["parameters"] = mergeStringMaps(toStringMap(result["parameters"]), paramsOverlay)
	}
	return result
}

func mergeStringMaps(base, overlay map[string]interface{}) map[string]interface{} {
	if base == nil {
		return overlay
	}
	if overlay == nil {
